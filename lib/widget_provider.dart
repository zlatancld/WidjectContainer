...
class WidgetProvider{
  ...
  T _getWidget<T extends Widget>({Key? key, dynamic args}) {
    ...
    if(widget == null) throw Exception("Widget $T hasn't been registered and can't be resolved.");

    return widget;
  }

  Future<T> getInitializedWidget<T extends Widget>({Key? key, dynamic args}) async{
    var scope = _tryGetScope<T>();
    if(scope != null) return await scope.getInitializedWidget(key: key, args: args);
